#!/usr/bin/env python3
"""Learn a bounded symmetric temporal binding width around a resident lag center."""
...
class HeadingCalibrationLearnedWindowV1:
    def __init__(self):
    ...
    def _tick(organism):
        ...
        if tick<0:raise ValueError('heading_window:organism')
        return tick
    ...
    def learned_width(self):
        center=self.learned_center()
        if center is None:return None
        counts=self._counts();width=0
        for distance in range(1,MAX_LAG+1):
            if abs(center-distance)>MAX_LAG or abs(center+distance)>MAX_LAG:break
            if counts.get(center-distance,0)>=FLANK_QUORUM and counts.get(center+distance,0)>=FLANK_QUORUM:
                width=distance
            else:break
        return width
    ...
    def _settle(self):
        ...
        center=self.learned_center();width=self.learned_width()
        accepted=(center is not None and width is not None and abs(lag-center)<=width)
        if accepted:
    ...
    def observe_visual(self,organism,value:int):
        ...
        if self._visual is not None and self._visual[0]==tick:raise ValueError('heading_window:duplicate_visual')
        self._visual=(tick,int(value));return self._settle()

    def observe_vestibular(self,organism,value:int):
        tick=self._tick(organism)
        if self._vestibular is not None and self._vestibular[0]==tick:raise ValueError('heading_window:duplicate_vestibular')
    ...
    def restore(cls,data):
        if int(data.get('schema',0))!=1:raise ValueError('heading_window:checkpoint')
        out=cls();out.lag_history=[int(x) for x in data.get('lag_history',())]
        if len(out.lag_history)>LAG_HISTORY or any(abs(x)>MAX_LAG for x in out.lag_history):raise ValueError('heading_window:checkpoint')
