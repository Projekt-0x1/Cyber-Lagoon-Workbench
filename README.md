...
`./verify` checks the workshop/export boundary, the no-build inner loop, the continuing checkpoint, and a focused reference falsifier without requiring a GPU. In a Git checkout it also requires the publication manifest to cover exactly the tracked public file set, excluding the manifest itself; an exported snapshot without `.git` still verifies every pinned byte. `./verify --refresh-manifest` is the explicit maintenance operation after an intentional tracked-file change: it atomically rebuilds and re-pins the tracked public file set, then exits. Manifest refresh is never part of the normal verify path and never auto-blesses a change.
