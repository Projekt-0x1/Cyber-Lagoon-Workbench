...
`./build` is the only normal compilation boundary. It materializes the last completed canonical checkpoint mark of the continuing reference Adult; if the current experimental curriculum tail is RED, that failure is retained in `.state/adult.provenance.json` rather than blocking access to the completed Adult. When a compatible CUDA toolchain/device is available, it also builds the minimal Direct dependency closure (Life Function + Adult Core + sit-down + fixed Resident Recipe IR evaluator) once, then births the Direct Adult once. Use `./build --require-direct` when lack of the Direct backend must be fatal.

`./adult` opens the continuing reference Adult through Claude Code as a body/client connected to a localhost Adult gateway. Claude is not called as the cognition model: the gateway response is produced by the Adult. The body process runs from a private Workbench directory with a narrow environment and Claude Code bare mode so repository/user hooks, plugins, MCP discovery, auto-memory, and `CLAUDE.md` are outside the transport boundary. `./adult --prompt "..."` sends one normal-sentence contact through that same Claude Code body and exits; `./adult --terminal` exposes the raw reference terminal body. The real Direct Adult is available for compile-free physical work through `./bench`; `./adult --backend direct --prompt "..."` is intentionally one-shot experimental until independent partner/world consequence settlement is proved for a continuing Direct Claude dialogue.
...
`./verify` checks the workshop/export boundary, the no-build inner loop, the continuing checkpoint, and a focused reference falsifier without requiring a GPU. `./verify --refresh-manifest` is the explicit maintenance operation after an intentional change to the already-published file set; it atomically re-pins those current bytes and exits. Manifest refresh is never part of the normal verify path and never auto-blesses a change.
