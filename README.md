...
`./adult` opens the continuing reference Adult through Claude Code as a body/client connected to a localhost Adult gateway. Claude is not called as the cognition model: the gateway response is produced by the Adult. `./adult --prompt "..."` sends one normal-sentence contact through that same Claude Code body and exits; `./adult --terminal` exposes the raw reference terminal body. The real Direct Adult is available for compile-free physical work through `./bench`; `./adult --backend direct --prompt "..."` is intentionally one-shot experimental until independent partner/world consequence settlement is proved for a continuing Direct Claude dialogue.
...
`./verify` checks the workshop/export boundary, the no-build inner loop, the continuing checkpoint, and a focused reference falsifier without requiring a GPU.
